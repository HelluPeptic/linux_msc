...
FORGE_INSTALLER_URL="https://maven.minecraftforge.net/net/minecraftforge/forge/26.2-65.1.0/forge-26.2-65.1.0-installer.jar"
FORGE_INSTALLER_JAR="forge-26.2-65.1.0-installer.jar"
...
download_forge_server() {
    ...
    /opt/jdk-25/bin/java -jar "$FORGE_INSTALLER_JAR" --installServer
    ...
    printf '#!/usr/bin/env sh\n# Add custom program arguments (such as nogui) to the next line before the "$@" or pass them to this script directly\n/opt/jdk-25/bin/java @user_jvm_args.txt @libraries/net/minecraftforge/forge/%s-%s/unix_args.txt "$@"\n' "$MINECRAFT_VERSION" "$FORGE_VERSION" > start.sh
