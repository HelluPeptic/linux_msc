...
build_folia() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f "$folia_jar_path" ]; then
            ...
            paper_jar_path="$SERVER_DIR/paper-1.21.1.jar"
            ...
            mv "$folia_jar_path" "$SERVER_DIR/folia-1.21.1.jar"
            echo "Folia jar successfully moved and renamed to folia-1.21.1.jar."
